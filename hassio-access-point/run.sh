...
CLIENT_INTERNET_ACCESS_INTERFACE=$(jq --raw-output ".client_internet_access_interface" $CONFIG_PATH)
...
if [ $DHCP -eq 1 ]; then
    ...
    if [ $CLIENT_INTERNET_ACCESS -eq 1 ]; then
        ...
        ## Route traffic
        iptables-nft -t nat -A POSTROUTING -o $CLIENT_INTERNET_ACCESS_INTERFACE -j MASQUERADE
